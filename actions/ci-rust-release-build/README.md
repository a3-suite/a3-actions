...
- uses: a3-suite/a3-actions/actions/ci-rust-release-build@<40-char-commit-sha>
  with:
    ...
    release-version-prefix: 'example-cli '
    release-asset-prefix: example-cli
```

入力と authority、checkout、manifest digest、manifest 内の platform ID／target、Cargo package version、
binary version が一致しない場合、toolchain が完全固定版ではない場合、
または出力先が既に存在する場合は失敗します。処理本体は同じ ref の
